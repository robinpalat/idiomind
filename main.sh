...
class Indicator:
    MENUCONF_PATH = os.getenv('HOME') + '/.config/idiomind/s/4.cfg'
    def __init__(self):
        self.ind = appindicator.Indicator('/usr/share/idiomind/images/flags/'+ 'en' + '.png',
        '/usr/share/idiomind/images/flags/'+ 'en' + '.png', appindicator.CATEGORY_APPLICATION_STATUS)
    ...
    def update_menu(self, widget = None, data = None):
        try:
            m = open(self.MENUCONF_PATH).readlines()
            menutopic = m
        except IOError:
            menutopic = []        
        menu = gtk.Menu()
        ...
        #if not os.path.isfile('/tmp/.p_'):
            #item = self.create_menu_label("Play") 
            #item.connect("activate", self.on_Play_click, 'Play:')
        #else:
            #item = self.create_menu_label("Stop") 
            #item.connect("activate", self.on_Stop_click, 'Stop:')
    ...
    def on_home(self, widget, path):
        os.system("'idiomind' 'topic'")

    def on_Play_click(self, widget, path):
        self.on_topic_changed()
    
    def on_Stop_click(self, widget, path):
        self.on_topic_changed()
